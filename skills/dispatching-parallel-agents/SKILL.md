...
has disjoint files, interfaces, fixtures, and mutable state. Astra first
...
review, re-review, or a second opinion. Those remain in the main Astra
session. A worker may not dispatch its own children.
...
Every call explicitly uses `gpt-5.6-luna`, `reasoning_effort = "xhigh"`, and
`fork_turns = "none"`. Make the calls concurrently only after confirming no
write or state dependency can overlap.

When all workers return, Astra reads every actual diff and report, checks for
...
fixer. If two failed fix attempts have the same root cause, Astra stops retrying,
...
- Integration order and any merge conflict resolution are known to Astra.
- Hardware, calibration, permissions, and external side effects remain under
  Astra's review and the user's existing authorization.
