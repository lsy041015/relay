...
1. Verify the approved plan, repository instructions, worktree, branch, and
   current user changes. Use `superpowers-astra-luna:using-git-worktrees` when
   isolation is required; never erase unrelated edits.
2. Resolve the plan workspace with
   `superpowers-astra-luna:subagent-driven-development`'s
...
4. Load `superpowers-astra-luna:test-driven-development` before Task 1 and
   apply `superpowers-astra-luna:verification-before-completion` before any
   completion claim.
...
- For a command with an `Expected:` line, apply
  `superpowers-astra-luna:verification-before-completion`: inspect matching
...
Use `superpowers-astra-luna:requesting-code-review` and its worksheet as a
...
`superpowers-astra-luna:verification-before-completion` to reuse matching
...
`superpowers-astra-luna:finishing-a-development-branch` only after the user
